...
// 美式
export const COLUMN_SERIALIZE_TYPES_EN_UK: DatePickerColumnType[] = [
  'Y',
  'M',
  'D',
  'h',
...
// 英式
export const COLUMN_SERIALIZE_TYPES_EN_US: DatePickerColumnType[] = [
  'D',
  'M',
  'Y',
  'h',
...
export const serializeMode = (
  ...
  // 起点终点
  const startIndex = COLUMN_SERIALIZE.findIndex((k) => k === modes[0]);
  const endIndex = COLUMN_SERIALIZE.findIndex((k) => k === modes[1]);
  const serializeModes: DatePickerColumnType[] = [];

  for (let index = startIndex; index <= endIndex; index++) {
    serializeModes.push(COLUMN_SERIALIZE[index] as DatePickerColumnType);
  }
...
export const formatDate = (
  ...
  day: Date,
  lang: LangType = LangType.zh_CN
) => {
  const dayDateObject = toDateObject(day);
  const modes = serializeMode(mode.split('-') as DatePickerColumnType[], lang);
