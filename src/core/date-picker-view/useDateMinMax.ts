...
import { useLangType } from '../../components/Locale/locale';
...
const useDateMinMax = (
  ...
  const [defaultMinDate, defaultMaxDate] = useMemo(() => {
    ...
    const _defaultMinDate = new Date(day.setFullYear(currentYear - 20));
    const _defaultMaxDate = new Date(day.setFullYear(currentYear + 20));
