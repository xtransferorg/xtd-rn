...
export const varCreator = (TOKENS: TokensType) => {
  return {
    ...
    button_option_disabled_text_color: TOKENS['--color-text-2'],
    button_prefix_margin: TOKENS['--spacing-1'],
  };
};
...
export const styleCreator = (cv: ComponentVars) => {
  return StyleSheet.create({
    ...
    button_disabled_text: {
      color: cv.button_option_disabled_text_color,
    },
    ...
    prefix: {
      marginRight: cv.button_prefix_margin,
    },
    prefix_disabled: {
      // opacity: cv.button_disabled_opacity, // 最外层设置透明度了
    },
  });
};
