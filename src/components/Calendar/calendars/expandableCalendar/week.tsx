...
import { CustomDayContext } from '../../Day';
...
function arePropsEqual(prevProps: WeekProps, nextProps: WeekProps) {
  ...
  return (
    ...
    isEqual(
      omit(prevOthers, ['customHeaderTitle']),
      omit(nextOthers, ['customHeaderTitle'])
    )
  );
}
...
const Week = React.memo((props: WeekProps) => {
  ...
  const { highlightDay } = useContext(CustomDayContext);
