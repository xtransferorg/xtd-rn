...
export const KNOB_CONTAINER_HEIGHT = 12;
...
export default function styleConstructor(theme: Theme = {}) {
  ...
  return StyleSheet.create({
    ...
    weekContainer: {
      ...
      left: 0,
      right: 0,
      height: (appStyle.headerHeight ?? 42) + 4,
      paddingTop: 1,
      top:
        HEADER_HEIGHT +
        (constants.isAndroid ? 10 : 8) -
        (appStyle.hideHeader ? appStyle.headerHeight ?? 44 : 0), // align row on top of calendar's first row
      backgroundColor: appStyle.calendarBackground,
    },
    hidden: {
      opacity: 0,
