...
export interface ExpandableCalendarProps extends CalendarListProps {
  ...
  /**
   * 禁用平移手势并禁用日历打开和关闭的选项（initialPosition 将保留）
   */
  disablePan?: boolean;
  /**
   *  是否隐藏下方旋钮
   ...
   * @default 46
   */
  constantWeekHeight?: number;
  /**
   * 周试图时的高度
   * @default 120
   */
  constantClosedHeight?: number;
  /**
   * 禁用滚动更改日期
   * @default true
   */
...
const ExpandableCalendar = (props: ExpandableCalendarProps) => {
  ...
  const { date, setDate, numberOfDays, timelineLeftInset } = context;
  const {
    ...
    constantWeekHeight = WEEK_HEIGHT,
    constantClosedHeight = CLOSED_HEIGHT,
    disableScrollChangeDate = true,

    visibleDate: outVisibleDate,
  ...
  useEffect(() => {
    if (outVisibleDate) {
      scrollToDate(outVisibleDate);
      // 根据时间计算出日历的高度
      const _numberOfWeeks = getNumberOfWeeksInMonth(outVisibleDate);
  ...
  const getOpenHeight = () => {
    ...
    return (
      constantClosedHeight +
      constantWeekHeight * (numberOfWeeks.current - 1) +
      (hideKnob ? 12 : KNOB_CONTAINER_HEIGHT) +
      (constants.isAndroid ? 8 : 4)
    );
  };
  ...
  const handleScreenReaderStatus = (_: boolean) => {
    // setScreenReaderEnabled(_);
  };
  ...
  const scrollToDate = (date: string) => {
    if (!horizontal) {
      ...
      visibleYear.current = getYear(date);
      visibleMonth.current = getMonth(date);
    }
  };
        ...
        if (month && visibleMonth.current !== month) {
          ...
          setTimeout(() => {
            ...
            if (_numberOfWeeks !== numberOfWeeks.current) {
              if (isOpen) {
                // 修复 knob 位置不正确
                _wrapperStyles.current.style.height = openHeight.current;
                updateNativeStyles();
              }
  ...
  const renderKnob = () => {
    return (
      <View
        style={StyleSheet.flatten([style.current.knobContainer, theme?.knob])}
    ...
    return (
      ...
      >
        <WeekComponent
          ...
          firstDay={firstDay}
          {...others}
          disableScrollChangeDate={disableScrollChangeDate}
