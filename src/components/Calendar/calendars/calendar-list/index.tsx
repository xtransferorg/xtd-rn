...
import { FlatListProps, View, ViewStyle, VirtualizedList } from 'react-native';
...
class CalendarItem extends React.Component<any> {
  shouldComponentUpdate(nextProps: {
    item: XDate;
    currentMonth: XDate;
    visible: boolean;
  }): boolean {
    const date = xdateToData(nextProps.currentMonth);
    const currentDate = xdateToData(this.props.item);
    const update =
      date.year === currentDate.year && date.month === currentDate.month;
    return update || nextProps.visible;
  }

  render() {
    // @ts-ignore
    return <CalendarListItem {...omit(this.props, 'currentMonth')} />;
  }
}
